...
namespace FEngSpriteGenerator
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"\t--{ArgumentKey.PaddingValue}          \t- Optional. Padding around images in the sprite. Defaults to '{defaultPadding}' pixels.");
            ...
            CustomConsoleMessage($"Padding around images: '{padding}'px", Console.ForegroundColor);
            ...
            int spriteWidth = (align == AlignSprite.Vertical) 
                    ? images.Max(image => image.Width + doublePadding)
                    : images.Sum(image => image.Width + doublePadding);
            int spriteHeight = (align == AlignSprite.Vertical) 
                    ? images.Sum(image => image.Height + doublePadding)
                    : images.Max(image => image.Height + doublePadding);
            ...
            using (Image<Rgba32> sprite = new Image<Rgba32>(spriteWidth, spriteHeight))
            {
                ...
                for (int i = 0; i < images.Length; i++)
                {
                    ...
                    // Create style from left-top angle of image minus padding value.
                    AppendCss(cssDocument, imageFile, cssSpriteNameFormat, x - padding, y - padding);
